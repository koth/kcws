...
# @Last Modified time: 2017-02-01 15:23:12
...
tf.app.flags.DEFINE_string('train_data_path', "pos/pos_train.txt",
                           'Training data dir')
tf.app.flags.DEFINE_string('test_data_path', "pos/pos_test.txt",
                           'Test data dir')
...
def do_load_data(path):
  ...
  for line in fp.readlines():
    ...
    if not line:
      continue
    ss = line.split(" ")
    if len(ss) != (FLAGS.max_sentence_len * (2 + FLAGS.max_chars_per_word)):
    ...
    for i in range(FLAGS.max_sentence_len):
      ...
      for k in range(FLAGS.max_chars_per_word):
        lcx.append(int(ss[FLAGS.max_sentence_len + i * FLAGS.max_chars_per_word
                          + k]))
      ly.append(int(ss[i + FLAGS.max_sentence_len * (FLAGS.max_chars_per_word +
                                                     1)]))
    wx.append(lwx)
    cx.append(lcx)
    y.append(ly)
...
class Model:
  def __init__(self, distinctTagNum, w2vPath, c2vPath, numHidden):
    ...
    with tf.variable_scope('CNN_Layer') as scope:
      self.filter = tf.get_variable(
          "filters_1",
          shape=[2, FLAGS.embedding_char_size, 1, FLAGS.embedding_char_size],
  ...
  def char_convolution(self, vecs):
    ...
    pool1 = tf.nn.max_pool(
        conv1,
        ksize=[1, FLAGS.max_chars_per_word - FLAGS.char_window_size + 1, 1, 1],
        strides=[1, FLAGS.max_chars_per_word - FLAGS.char_window_size + 1, 1,
                 1],
        padding='SAME')
...
def read_csv(batch_size, file_name):
  ...
  decoded = tf.decode_csv(
      ...
      record_defaults=[
          [0]
          for i in range(FLAGS.max_sentence_len * (FLAGS.max_chars_per_word + 2
                                                   ))
      ])
        ...
        try:
          ...
          if (step + 1) % 1000 == 0:
            test_evaluate(sess, test_unary_score, test_sequence_length,
                          trainsMatrix, model.inp_w, model.inp_c, twX, tcX, tY)
